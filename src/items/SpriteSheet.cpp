...
long now() { return std::chrono::duration_cast< std::chrono::milliseconds >(std::chrono::system_clock::now().time_since_epoch()).count(); }
...
void SpriteSheet::onUserUpdate(PixEngine *engine, float fElapsedTime) {
	...
	// texture sampler
	pShader->setInt("sampler", pTexture->unit()); // RLP
	// projecyion matrix
	pShader->setMat4("projection", &mProjection[0][0]);
	// spritesheet metrics
	pShader->setVec4("iSpriteSheet", pTexture->width(), pTexture->height(), NUMX, NUMY);
