...
Canvas2D::Canvas2D(Drawable *target) : pTarget(target) {}

void Canvas2D::clear(Pixel color) {
	pTarget->clear(color);
}
...
void Canvas2D::drawTriangle(int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t x3, int32_t y3, Pixel p) {
...
void Canvas2D::drawWireFrameModel( const std::vector<std::pair<float, float>> &vecModelCoordinates, float x, float y, float r, float s, std::vector<rgl::Pixel> col)
{
